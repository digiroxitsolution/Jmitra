...
<head>
    ...
    <style>
        ...
        .scrollable-card-body {
            white-space: nowrap;
            overflow-x: hidden; /* Remove inner scrollbar */
        }
        .sticky-scrollbar {
            position: fixed;
            bottom: 0;
        ...
        .table thead tr {
            position: sticky;
            top: 0;
            background: white; /* Ensure header is not transparent */
...
<body>
					...
					<div class="card-dialog card-dialog-centered card-fullscreen">
						...
						<div class="card-header" style="background-color:#077fad; color:white;">
						  <h4 class="" style="height: 50px;">Monthly Expenses Report
						  	<a href="{{ route('pending_expense_verification_print', $pending_monthly_expenses->id) }}"><button type="submit" class="btn btn-success float-end"><i class="fa-solid fa-download"></i></button></a>
						...
						@include('includes.message')						
							<div class="row mb-4">
								...
								<div class="col-lg-4 col-12">
									Month: {{ \Carbon\Carbon::parse($pending_monthly_expenses->expense_date)->format('F') }}
			                ...
			                <div style="width: 3000px; background-color: lightblue;">
			                    <table class="table table-bordered table-hover text-center table-light" style="width: 2982px;">
                                    <thead style="position: sticky !important; top: 0% !important;">
                                        <tr class="table-active fw-bold" style="position: sticky !important; top: 0% !important;">
                                            <th rowspan="2">Date</th>
                                           <th rowspan="2">Day</th>
                                           <!--<th rowspan="2"style="width: 10%">Holiday</th>-->
                                        ...
                                        <tr class="table-active fw-bold">
                                            ...
                                            <tbody>                                                
                                                ...
                                                <form action="{{ route('pending_expense_verification_update.update', $pending_monthly_expenses->id) }}" method="POST">
                                                    @csrf
                                                    @method('PUT')

                                                    <input type="number" id="pending_monthly_expense_id" value="{{ $pending_monthly_expenses->id }}" name="pending_monthly_expense_id" hidden> 
                                                ...
                                                >
                                                    ...
                                                    <td class="history-cell">
                                                        <input class="form-control" type="date" id="expense_date_{{ $key }}" value="{{ \Carbon\Carbon::parse($monthly_expense->expense_date)->format('Y-m-d') }}" class="form-control no-border" name="monthly_expense[{{ $key }}][expense_date]">
    ...
    {{ \Carbon\Carbon::parse($monthly_expense->expense_date)->format('l') }}
                                                    ...
                                                    <td class="history-cell @foreach($monthly_expense_histories[$key] as $history)
                                                        @if($monthly_expense->from != $history->from)
                                                            edited
                                                                
                                                        @endif
                                                        @endforeach   ">
                                                        <input type="text" id="from_{{ $key }}" value="{{ $monthly_expense->from }}" class="form-control no-border" name="monthly_expense[{{ $key }}][from]">
                                                    ...
                                                    <td class="history-cell
                                                        ...
                                                        @endforeach
                                                    ">
                                                        <input type="text" id="to_{{ $key }}" value="{{ $monthly_expense->to }}" class="form-control no-border" name="monthly_expense[{{ $key }}][to]">
                                                    ...
                                                    @foreach($monthly_expense_histories[$key] as $history)
                                                        ...
                                                        @endforeach
                                                    ">
                                                        <input type="number" id="km_as_per_user_{{ $key }}" value="{{ $monthly_expense->km_as_per_user }}" class="form-control no-border no-negative" name="monthly_expense[{{ $key }}][km_as_per_user]">
                                                    ...
                                                    <td class="history-cell
                                                        ...
                                                        @endforeach
                                                    ">
                                                        <input type="number" id="km_as_per_google_map_{{ $key }}" value="{{ $monthly_expense->km_as_per_google_map }}" class="form-control no-border no-negative" name="monthly_expense[{{ $key }}][km_as_per_google_map]" readonly>
                                                    ...
                                                    <td class="history-cell
                                                        ...
                                                        @endforeach
                                                    ">
                                                        <select class="form-select no-border" aria-label="Default select example" id="mode_expense_{{ $key }}" name="monthly_expense[{{ $key }}][mode_expense]">
                                                    ...
                                                    <td>
                                                    	<select id="calculates_fare_{{ $key }}" name="calculates_fare{{ $key }}" class="form-select">
                                                    		<option selected disabled>Select</option>
                                                    		<option value="by user">KM as per User</option>
                                                    		<option value="by google map">KM as per Google Map</option>
                                                    	</select>
                                                    </td>
                                                    <td class="history-cell
                                                        @foreach($monthly_expense_histories[$key] as $history)
                                                        ...
                                                        @endforeach
                                                    ">
                                                        <input type="number" id="fare_amount_{{ $key }}" value="{{ $monthly_expense->fare_amount }}" class="form-control no-border no-negative" name="monthly_expense[{{ $key }}][fare_amount]">
                                                          
                                                    
                                                        @foreach($monthly_expense_histories[$key] as $history)
                                                        @if($monthly_expense->fare_amount != $history->fare_amount)
                                                        <div class="hover-box">
                                                            <div class="left">{{ $history->fare_amount }}</div>
                                                        </div>
                                                        @endif        
                                                        @endforeach
                                                    ...
                                                    <td class="history-cell
                                                        ...
                                                        @endforeach
                                                    ">
                                                        <input type="number" id="da_total_{{ $key }}" value="{{ $monthly_expense->expense_type_master_id != 8 ? $monthly_expense->da_total : '' }}" class="form-control no-border no-negative" name="monthly_expense[{{ $key }}][da_total]">
                                                    ...
                                                    @foreach($monthly_expense_histories[$key] as $history)
                                                        ...
                                                        @endforeach
                                                    ">
                                                        <input type="number" id="da_total_{{ $key }}" value="{{ $monthly_expense->expense_type_master_id == 8 ? $monthly_expense->da_total : '' }}"  class="form-control no-border no-negative" name="monthly_expense[{{ $key }}][expense_type_master_id]">
                                                    ...
                                                    @foreach($monthly_expense_histories[$key] as $history)
                                                        ...
                                                        @endforeach
                                                    ">
                                                        <input type="number" id="postage_{{ $key }}" value="{{ $monthly_expense->postage }}" class="form-control no-border no-negative" name="monthly_expense[{{ $key }}][postage]">
                                                    ...
                                                    @foreach($monthly_expense_histories[$key] as $history)
                                                        ...
                                                        @endif     
                                                        @endforeach">
                                                        <input type="number" id="mobile_internet_{{ $key }}" value="{{ $monthly_expense->mobile_internet }}" class="form-control no-border no-negative" name="monthly_expense[{{ $key }}][mobile_internet]">
                                                        @foreach($monthly_expense_histories[$key] as $history)
                                                        @if($monthly_expense->mobile_internet != $history->mobile_internet)
                                                            <div class="hover-box">
                                                                <div class="left">
                                                    ...
                                                    @foreach($monthly_expense_histories[$key] as $history)
                                                        ...
                                                        @endforeach
                                                        ">
                                                        <input type="text" id="print_stationery{{ $key }}" value="{{ $monthly_expense->print_stationery }}" class="form-control no-border" name="monthly_expense[{{ $key }}][print_stationery]">
                                                    ...
                                                    <td class="history-cell
                                                        ...
                                                        @endforeach
                                                    ">
                                                        <select class="form-select" aria-label="Default select example" id="other_expense_{{ $key }}" name="monthly_expense[{{ $key }}][other_expense]">
                                                    ...
                                                    @foreach($monthly_expense_histories[$key] as $history)
                                                        ...
                                                        @endforeach
                                                        ">
                                                        <input type="text" id="other_expenses_amount_{{ $key }}" value="{{ $monthly_expense->other_expenses_amount }}" class="form-control no-border" name="monthly_expense[{{ $key }}][other_expenses_amount]">
                                                ...
                                                <tr class="table-active">
                                                    <td colspan="9" style="width: 8%" class="fw-bold text-end">Total</td>
                                                    <td><strong>Fare:</strong> <span id="totalFare">₹ 0.00</span></td>
                                                    <td colspan="2"><strong>Working:</strong> <span id="totalWorking">₹ 0.00</span></td>
                                                    
                                                    <td><strong>Postage:</strong> <span id="totalPostage">₹ 0.00</span></td>
                                                    <td><strong>Tel/TGM:</strong> <span id="totaltel_tgm">₹ 0.00</span></td>
                                                    <td></td>
                                                    <td><strong>Amount:</strong> <span id="totalAmount">₹ 0.00</span></td>
                                                    <td><strong>Stationery:</strong> <span id="totalStationery">₹ 0.00</span></td>
                                                    <td></td>
                                                </tr>
                                                <tr class="table-active">
							...
							<div class="row mb-4">
								...
								<div class="col-lg-4 col-12">Approved by<br>{{ $pending_monthly_expenses->approvedBy?->name }}<br>
						...
						<div class="card-footer" style="height: 50px;">
							<a href="{{ route('pending_expense_verification.index')}}"><button type="button" class="btn btn-danger float-end ms-2" data-bs-dismiss="modal">Closed</button></a>
							@can('monthly-pending-expense-reject')
							
							    <button type="button" data-bs-toggle="modal" 
							        data-bs-target="#reasonOfRejectedModal" class="btn btn-danger float-end ms-2">Rejected</button>
							...
							@can('monthly-pending-expense-reopen')
							    ...
							    <button type="button" data-bs-toggle="modal" 
							        data-bs-target="#reasonOfReOpenModal" class="btn btn-warning text-white float-end ms-2">Re-Open</button>
							
							@endcan

							@can('monthly-pending-expense-update')
							<button type="button" class="btn btn-info float-end ms-2 text-white" id="externalSubmitButton">Update</button>
							@endcan
							...
							@can('monthly-pending-expense-verify')
							    ...
							    <button type="button"  data-bs-toggle="modal" 
							        data-bs-target="#submissionPromisiingModal" class="btn btn-success float-end ms-2">Verify & Submit</button>
							...
							@can('monthly-pending-expense-approve')
							    ...
							    <button type="button" data-bs-toggle="modal" 
							        data-bs-target="#submissionPromisiingModal" class="btn btn-success float-end ms-2">Approve & Submit</button>
					...
					<div class="modal-dialog modal-dialog-centered">
					  <div class="modal-content">
						...
						<div class="modal-footer">
							@can('monthly-pending-expense-approve')
							<a href="{{ route('pending_expense_verification.changeStatus', ['id' => $pending_monthly_expenses->id, 'status' => 1]) }}">
							    <button type="button"  class="btn btn-success float-end ms-2" {{ $pending_monthly_expenses->is_approve === 1 ? 'disabled' : '' }}>Final Approve & Submit</button>
							</a>
							@endcan
							@can('monthly-pending-expense-verify')
							<a href="{{ route('pending_expense_verification.changeStatus', ['id' => $pending_monthly_expenses->id, 'status' => 2]) }}">
							    <button type="button"  class="btn btn-success float-end ms-2">Verify & Submit</button>
							</a>
							@endcan
				...
				<div class="modal" tabindex="-1" id="reasonOfRejectedModal">
					<div class="modal-dialog modal-dialog-centered">
					  <div class="modal-content">
						...
						<form action="{{ route('pending_expense_reject', $pending_monthly_expenses->id) }}" method="POST">
						    @csrf
						    <div class="modal-body">
						        <label for="rejection_master_id">Reason for Rejection</label>
						        <select name="rejection_master_id" id="rejection_master_id" class="form-select" aria-label="Select reason for rejection" required>
						            <option value="" selected disabled>Select Reason</option>
						            @foreach($reason_of_rejections as $reason_of_rejection)
						                <option value="{{ $reason_of_rejection->id }}">{{ $reason_of_rejection->reason_of_rejection }}</option>
						            @endforeach
						        </select>
						    </div>
						    <div class="modal-footer">
				...
				<div class="modal" tabindex="-1" id="reasonOfReOpenModal">
					<div class="modal-dialog modal-dialog-centered">
					  <div class="modal-content">
						...
						<form action="{{ route('pending_expense_re_open', $pending_monthly_expenses->id ) }}" method="POST">
							@csrf
							<div class="modal-body">
								<label for="re_open_master_id">Reason of Re-Open</label>
								<select name="re_open_master_id" id="re_open_master_id" class="form-select" aria-label="Default select example" required>
									<option selected>Select Reason of Re-Open</option>
									@foreach( $reason_of_re_opens as $reason_of_re_open )
									<option value="{{ $reason_of_re_open->id}}">{{ $reason_of_re_open->reason_of_re_open }}</option>
									@endforeach
								</select>
							</div>
							<div class="modal-footer">
...
<script type="text/javascript">
    ...
    <script type="text/javascript">
		...
		//     let print_stationeryInputs = document.querySelectorAll("[id^='print_stationery_']");
 		...
 		$(document).ready(function() {
		    $(document).on('change', "[id^='mode_expense_'], [id^='calculates_fare_'], [id^='km_as_per_user_'], [id^='km_as_per_google_map_']", function() {
		    ...
		    $("[id^='mode_expense_'], [id^='calculates_fare_'], [id^='km_as_per_user_'], [id^='km_as_per_google_map_']").each(function() {
		        $(this).trigger('change');
		    });
		});
		...
		fromInputs.forEach((fromInput, index) => {
			...
			const checkAndTriggerDistanceCalculation = () => {
				if (fromInput.value.trim() !== "" && toInput.value.trim() !== "") {
					console.log(`Both "From" and "To" fields are filled for set ${index + 1}. Calculating distance...`);
					setTimeout(() => {
...
async function fetchDistanceMatrixOnClick(event) {
    ...
    try {
        ...
        if (response.ok) {
            ...
            if (distance && distance.value) {
                row.querySelector('[id^="km_as_per_google_map_"]').value = (distance.value / 1000).toFixed(2); // Set the distance in the km field
            } else {
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    function calculateGrandTotal() {
        ...
        let grandTotal = totalFare + totalWorking + totalPostage + totalTelTGM + totalAmount + totalStationery;
