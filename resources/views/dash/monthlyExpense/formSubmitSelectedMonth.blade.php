			...
			<div class="row">
				...
				<div class="card">
					<div class="card-dialog card-dialog-centered card-fullscreen">
						...
						<div class="card-header" style="background-color:#077fad; color:white;">
						  <h4 class="" style="height: 50px;">Form Submit of Selected Month
						  	@if($user_expense_other_record->is_submitted === 1)
							...
							<div class="row mb-4">
								<table class="table table-bordered table-hover text-center table-light">
									<tbody>
										...
										<tr>
											<td style="width:150px;">{{ \Carbon\Carbon::parse($monthly_expense->expense_date)->format('d-m-Y') }}</td>
											<td>
										...
										<td style="width:150px;">
											...
											<td style="width:150px;">
											    {{ $monthly_expense->departure_time ? \Carbon\Carbon::parse($monthly_expense->arrival_time)->format('d-m-Y') : '' }}
											    
												
												<hr>
												{{ $monthly_expense->departure_time ? \Carbon\Carbon::parse($monthly_expense->arrival_time)->format('h:i A') : '' }}
											</td>
											...
											<td>
                                                @unless($monthly_expense->expense_type_master_id == 8)
                                                    {{ $monthly_expense->da_total ?? 'N/A' }}
                                                @else
                                                    N/A
                                                @endunless
                                            </td>
											...
											<td>{{ $monthly_expense->mobile_internet ?? 'N/A' }}</td>
									...
									<tbody>
										...
										<form action="{{ route('user_expense_other_record_update.update', $user_expense_other_record->id) }}" method="POST">
											...
											<tr>
												...
												<td colspan="2">
													<input type="number" class="form-control" min="0" value="{{ $user_expense_other_record->advance_taken }}" name="advance_taken" id="advance_taken" {{ $user_expense_other_record->is_submitted === 1 ? 'disabled' : '' }} ></td>
											</tr>
											<tr>
												<td class="text-primary">Remark of Advance Taken</td>
												<td colspan="2">
													<input id="advance_taken" type="text" class="form-control" value="{{ $user_expense_other_record->remark_of_advance_taken }}" id="remark_of_advance_taken" name="remark_of_advance_taken" {{ $user_expense_other_record->is_submitted === 1 ? 'disabled' : '' }} >
												</td>
											</tr>
										...
										<tr>
											...
											<td colspan="2"><input type="text" class="form-control" value="₹ {{ $balance_dues }}" id="balance_dues" disabled></td>
										</tr>
									</tbody>
								</table>
							</div>
					...
					<div class="modal-dialog modal-dialog-centered">
					  <div class="modal-content">
						...
						<div class="modal-footer">
							<button type="button" id="externalSubmissionButton" class="btn btn-success">Final Submit</button>
						</div>
					  </div>
					</div>
				  </div>
				...
				<div class="modal" tabindex="-1" id="justificationModal">
					<div class="modal-dialog modal-dialog-centered">
					  <div class="modal-content">
						...
						<div class="modal-footer">
							<button type="button" data-bs-toggle="modal" data-bs-target="#submissionPromisiingModal" class="btn btn-success">Final Submit</button>
						</div>
					  </div>
					</div>
				  </div>
				...
				*****************-->
						...
						<div class="card-footer" style="height: 50px;">
							...
							@if ($user_expense_other_record->days_elapsed != 0)
							    <button 
							        type="button" 
							        class="btn btn-success float-end ms-2" 
							        data-bs-toggle="modal" 
							        data-bs-target="#justificationModal" 
							        {{ $user_expense_other_record->is_submitted === 1 ? 'disabled' : '' }}>
							        Submit
							    </button>
							@else
							    <button 
							...
							<button type="button" class="btn btn-info text-light float-end  ms-2" id="externalSubmitButton" {{ $user_expense_other_record->is_submitted === 1 ? 'disabled' : '' }} >Save</button>
...
document.addEventListener('DOMContentLoaded', function () {
    document.querySelector('table').addEventListener('click', function (event) {
        if (event.target && event.target.classList.contains('viewAttendanceButton')) {
            ...
            console.log('Check-in value:', check_in); // Log check-in value
            ...
            fetch(url)
                ...
                .then(data => {
                    ...
                    // Populate rows with formatted attendance data
...
<script>
document.addEventListener("DOMContentLoaded", function () {
    const advanceTakenInput = document.getElementById("advance_taken");
    const balanceDuesInput = document.getElementById("balance_dues");
    const grandTotal = parseFloat("{{ $grand_total }}") || 0; // Get grand total from PHP

    function updateBalanceDues() {
        let advanceTaken = parseFloat(advanceTakenInput.value) || 0;
        let balanceDues = grandTotal - advanceTaken;

        // Format balance dues with currency symbol
        balanceDuesInput.value = `₹ ${balanceDues.toFixed(2)}`;
    }

    // Trigger update when input changes
    advanceTakenInput.addEventListener("input", updateBalanceDues);
