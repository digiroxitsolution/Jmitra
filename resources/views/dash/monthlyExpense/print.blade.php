        ...
        <div>
            ...
            <div>
                <table style="width: 100%; border-collapse: collapse;">
                    <tr>
                        <td style="padding: 10px; min-width: 200px;">Company Name: @if (Auth::user()->userDetail && Auth::user()->userDetail->companyMaster->company_name )
	                                    {{ Auth::user()->userDetail->companyMaster->company_name  }}
		                                @else
		                                    N/A
		                                @endif
                        </td>
                        <td style="padding: 10px; min-width: 200px;">Division: @if (Auth::user()->userDetail && Auth::user()->userDetail->DivisonMaster->name )
                        ...
                        <td style="padding: 10px; min-width: 200px;">Month: {{ \Carbon\Carbon::parse($month)->format('F') }}</td>
                    </tr>
                    <tr>
                        <td style="padding: 10px; min-width: 200px;">Name: @if (Auth::user()->userDetail && Auth::user()->name)
	                                    {{ Auth::user()->name }}
	                                @else
	                                    N/A
	                                @endif</td>
                        <td style="padding: 10px; min-width: 200px;">Designation: @if (Auth::user()->userDetail && Auth::user()->userDetail->designation->name )
	                                    {{ Auth::user()->userDetail->designation->name  }}
		                                @else
		                                    N/A
		                                @endif
                        </td>
                        <td style="padding: 10px; min-width: 200px;">Location: @if (Auth::user()->userDetail && Auth::user()->userDetail->locationMaster->working_location )
                        ...
                        <table style="width: 100%; border-collapse: collapse; margin-bottom: 20px; text-align: center;">
                            <tbody>
                                <tr style="background-color: #E5E6E7">
                                    <td style="border: 1px solid #a8acb1; padding: 0px 0px 0px 0px" rowspan="2">Date</td>
                                    ...
                                    <td style="border: 1px solid #a8acb1; padding: 5px 10px 10px 15px" rowspan="">KM as per User</td>
                                    <!--<td style="border: 1px solid #a8acb1; padding: 5px 10px 10px 15px" rowspan="2">KM as per Google</td>-->
                                ...
                                <tr>
                                    <td style="border: 1px solid #a8acb1; padding: 0px 0px 0px 0px">{{ \Carbon\Carbon::parse($monthly_expense->expense_date)->format('Y-m-d') }}</td>
                                    ...
                                    <td style="border: 1px solid #a8acb1; padding: 5px 10px 10px 15px">{{ $monthly_expense->km_as_per_user }}</td>

                                    <!--<td style="border: 1px solid #a8acb1; padding: 5px 10px 10px 15px">{{ $monthly_expense->km_as_per_google_map }}</td>-->
                    ...
                    <tr>
                        <td colspan="2" style="padding: 10px;">
                            <input class="form-check-input me-2 ms-1" type="checkbox" value="1" id="accept_policy" name="accept_policy" {{ $monthly_expense->accept_policy == 1 ? 'checked' : '' }} {{ $monthly_expense->is_submitted === 1 ? 'disabled' : '' }} >
                    ...
                    <tr>
                        <td style="padding: 10px; min-width: 200px;">Submitted by<br>
                            {{ $monthly_expense->user?->name }}<br>

                                    {{ $monthly_expense->expense_date ? \Carbon\Carbon::parse($monthly_expense->expense_date)->format('d-m-Y') : '' }}
                                    <br>
                                    {{ $monthly_expense->expense_date ? \Carbon\Carbon::parse($monthly_expense->expense_date)->format('h:i A') : '' }}
                        </td>

                        <td style="padding: 10px; min-width: 200px;">Verified by<br>
                            {{ $user_expense_other_record->verifiedBy?->name ?? 'N/A' }}<br>
