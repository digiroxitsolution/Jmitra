						...
						<table id="example" class="table bg-white rounded shadow-sm table-hover" >
						  <thead>
						    <tr>
							  ...
							  <th scope="col">Expense Verification Date</th>
						      <th scope="col">Expense Approved Date</th>
						      <th scope="col">Days Elapsed (Submission)</th>
							  <th scope="col">Days Elapsed (Verification)</th>
							  <th scope="col">Days Elapsed (Approval)</th>
						    </tr>
						  </thead>
						  <tbody>
						  	@php $i=1 @endphp
						    ...
						    <tr>
							  ...
							  <td>{{ $monthly_expense->verified_time ? \Carbon\Carbon::parse($monthly_expense->verified_time)->format('d-m-Y') : 'N/A' }}</td>

							  <td>{{ $monthly_expense->approved_time ? \Carbon\Carbon::parse($monthly_expense->approved_time)->format('d-m-Y') : 'N/A' }}</td>

							  <td>{{ $monthly_expense->days_elapsed }}</td>

							  <td>{{ $monthly_expense->verification_days_elapsed ?? 'N/A' }}</td>
							  <td>{{ $monthly_expense->approval_days_elapsed ?? 'N/A' }}</td>
						    </tr>
						    @endforeach
						  </tbody>
						</table>
